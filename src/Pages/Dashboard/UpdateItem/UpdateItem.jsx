  ...
  const onSubmit = async (data) => {
    try {
      ...
      const updatedMenuItem = {
        name: data.name,
        recipe: data.recipe,
        image: imageUrl || image, // Use new image URL or keep existing one
      ...
      // Update the item in database using PUT/PATCH request
      const res = await axiosSecure.patch(`/menu/${_id || id}`, updatedMenuItem);
      ...
      if (res.data.modifiedCount > 0) {
        ...
        // Refetch menu data to update the UI
        refetch();
      } else {
        Swal.fire({
          title: "Info",
          text: "No changes were made to the menu item.",
          icon: "info",
          confirmButtonText: "OK"
        });
      }
  ...
  return (
    <div className="min-h-screen bg-white p-6">
      ...
      <div className="text-center mb-8">
        <p className="text-orange-400 text-sm mb-2">---Update Item---</p>
        ...
        <div className="bg-[#F3F3F3] rounded-lg shadow-sm p-8">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="pt-4">
              ...
              >
                <ChefHat size={18} />
                {loading ? "Updating..." : "Update Recipe Details"}
              </button>
            </div>
          </form>
        </div>
